...
const UnselectedRecipeFooter = ({
  ...
  <Flex p="xs">
    <Box flex="50%" alignSelf="center">
      {isExtra ? <Text color="primary_600">+{parseRawPrice(price)}</Text> : null}
    </Box>
    <Box flex="50%">
      <Button
...
const SelectedRecipeFooter = ({
  ...
  <Flex data-testid="selected-footer" backgroundColor="primary_600" justifyContent="space-between" alignItems="center">
    <SelectionButton data-testid="minus-button" onClick={() => handleRemoveRecipe(recipeId)} title="Decrease quantity">
      <IconMinusCircle />
    </SelectionButton>
    <Box color="white">
    ...
    <SelectionButton data-testid="plus-button" onClick={() => handleAddRecipe(recipeId)} title="Increase quantity" disabled={maxRecipesSelected || selected >= selectionLimit}>
      <IconPlusCircle />
    </SelectionButton>
  </Flex>
);
